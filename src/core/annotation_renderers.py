...
from core.annotation_system import AnnotationRenderer, Annotation
...
def render_keyword_annotation(annotation: Annotation, draw_context: ImageDraw.Draw, bounds: Dict[str, int]):
    """Render a keyword annotation as a highlighted pill."""
    x0, y0, x1, y1 = bounds['x0'], bounds['y0'], bounds['x1'], bounds['y1']
    color = annotation.color

    # Semi-transparent highlight
    highlight_color = hex_to_rgba(color, alpha=80)
...
def render_url_annotation(annotation: Annotation, draw_context: ImageDraw.Draw, bounds: Dict[str, int]):
    """Render a URL annotation as a status border with indicator."""
    x0, y0, x1, y1 = bounds['x0'], bounds['y0'], bounds['x1'], bounds['y1']
    color = annotation.color
...
class KeywordRenderer(AnnotationRenderer):
    ...
    def render_annotation(self, annotation: Annotation, draw_context: ImageDraw.Draw, bounds: Dict[str, int]):
        render_keyword_annotation(annotation, draw_context, bounds)
...
class URLRenderer(AnnotationRenderer):
    ...
    def render_annotation(self, annotation: Annotation, draw_context: ImageDraw.Draw, bounds: Dict[str, int]):
        render_url_annotation(annotation, draw_context, bounds)
